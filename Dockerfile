...
RUN go build -o /build-out/nakama -trimpath -mod=vendor -gcflags "-trimpath $PWD" -asmflags "-trimpath $PWD" -ldflags "-s -w"

FROM debian:buster-slim

MAINTAINER BergerBytes <contact@bergerbytes.io>
...
WORKDIR /nakama/
COPY --from=builder "/build-out/nakama" /nakama/
